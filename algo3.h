...
BitSeq encode3(unsigned w, unsigned h, uint8_t *r, uint8_t *g, uint8_t *b)
{
    BitSeq data;
    data.write_back(MAGIC_FILEID_NUMBER | 0x0301);
    data.write_back(h);
    data.write_back(w);
...
void decode3(BitSeq &data, unsigned &w, unsigned &h, uint8_t *&r, uint8_t *&g, uint8_t *&b)
{
    h = data.readUnsigned();
    w = data.readUnsigned();
